...
void help(void)
{
	...
	printf("\t-D            Skip disk configuration sections\n");
...
mvprintw(x+16, 3, "   h = more options                   q = Quit");
		...
		if (show_help && cursed) {
			...
			mvwprintw(padhelp, 7, 5, "m = Memory and Swap  stats       j = JFS Usage Stats");
			...
			mvwprintw(padhelp,19, 5, "Developer Nigel Griffiths");
			DISPLAY(padhelp,20);
...
#define IFDELTA_ZERO(member1,member2) ((IFDELTA(member1) == 0) || (IFDELTA(member2)== 0)? 0.0 : IFDELTA(member1)/IFDELTA(member2) )
				...
				CURSE mvwprintw(padnet,2 + i, 0, "%6s %7.1f %7.1f    %6.1f   %6.1f  %6.1f %6.1f    %7.1f %7.1f   ",
		...
		if(show_neterror) {
			...
			for (i = 0; i < networks; i++) {
				CURSE mvwprintw(padneterr,2 + i, 0, "%6s %7lu %7lu %7lu %7lu %7lu %7lu %7lu %7lu %7lu",
				    &p->ifnets[i].if_name[0],
				    p->ifnets[i].if_ierrs,   
				    p->ifnets[i].if_idrop,   
				    p->ifnets[i].if_ififo,   
...
*/
					...
					if(show_disk == SHOW_DISK_GRAPH) {
							...
							mvwprintw(paddisk,2 + i, 28+(int)(disk_busy_peak[i]/2), ">");
